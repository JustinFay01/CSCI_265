...
public class HammingTest {
	...
	public void testEncode2() {
		...
		assert(equal(bits1, "00000101011"));
		byte[] bits2 = hammy.encode('M', EVENPARITY);
		assert(equal(bits2, "10100111001"));
		byte[] bits3 = hammy.encode('!', ODDPARITY);
		assert(equal(bits3, "01110000010"));
		byte[] bits4 = hammy.encode(' ', EVENPARITY);
		assert(equal(bits4, "01000001010"));
	}
	...
	public void testDecodeNoErrors() {
		...
		assert(c1 == 'A');
		byte[] bits2 = {0, 0, 0, 0, 0, 1, 0, 1, 0, 1, 1};
		char c2 = hammy.decode(bits2, ODDPARITY);
		assert(c2 == 'd');
		byte[] bits3 = {1, 0, 1, 0, 0, 1, 1, 1, 0, 0, 1};
